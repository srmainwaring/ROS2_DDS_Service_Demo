...
#include "ArmMotorsInt64_service.hpp"

#include <ctime>
#include <chrono>


ArmMotorsInt64_Service::ArmMotorsInt64_Service(
    ...
    , tsType_req_(new ArmMotorsInt64_RequestPubSubType())
    , tsType_rep_(new ArmMotorsInt64_ReplyPubSubType())
    , server_(server)
    , stop_(false)
    , matched_mutex_()
    , mutex_()
...
std::future<ArmMotorsInt64_Reply> ArmMotorsInt64_Service::request(
        ArmMotorsInt64_Request& request)
{
    if (!server_)
    {
        ...
        delete promise_;
        promise_ = new std::promise<ArmMotorsInt64_Reply>();
...
void ArmMotorsInt64_Service::run(
        ...
        const uint32_t sleep)
{
    ...
    {
        stop_ = false;
        std::thread thread(&ArmMotorsInt64_Service::runThread, this, number, sleep);
        if (0 == number)
        {
...
void ArmMotorsInt64_Service::on_publication_matched(
...
void ArmMotorsInt64_Service::runThread(
        ...
        const uint32_t sleep)
{
    ...
    if (samples == 0)
    {
        ...
        while (!stop_)
        {
            ...
            std::cout << "\t- Request " << request.arm() << std::endl;
            ...
            {
                std::cout << "\t - Received response: " << response.get().result() << std::endl;
...
void ArmMotorsInt64_Service::RequestListener::publisher(
        ArmMotorsInt64_Service* service,
...
void ArmMotorsInt64_Service::RequestListener::on_subscription_matched(
...
void ArmMotorsInt64_Service::RequestListener::on_data_available(
        fastdds::dds::DataReader* reader)
{
    fastdds::dds::SampleInfo info;
    ArmMotorsInt64_Request request;
    ArmMotorsInt64_Reply response;
            ...
            reader->take_next_sample(&request, &info))
    {
...
#endif //  if FASTRTPS_VERSION_MINOR < 2
        {
            ...
            std::cout << service_->service_name_ << " server: processing incoming request, arm "
                      << request.arm() << std::endl;

            response.result(request.arm());

            std::cout << "\t - Sending back response: " << response.result() << std::endl;
...
void ArmMotorsInt64_Service::ReplyListener::publisher(
        ArmMotorsInt64_Service* service)
{
    service_ = service;
}

void ArmMotorsInt64_Service::ReplyListener::on_subscription_matched(
        fastdds::dds::DataReader* /*datareader*/,
        const fastdds::dds::SubscriptionMatchedStatus& /*info*/)
{
    std::unique_lock<std::mutex> lock(service_->matched_mutex_);
...
void ArmMotorsInt64_Service::ReplyListener::on_data_available(
        fastdds::dds::DataReader* reader)
{
