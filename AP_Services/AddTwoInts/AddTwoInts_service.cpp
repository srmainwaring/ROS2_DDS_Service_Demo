...
AddTwoInts_Service::AddTwoInts_Service(
    ...
    , tsType_rep_(new AddTwoInts_ReplyPubSubType())
    , server_(server)
    , stop_(false)
    , matched_mutex_()
    , mutex_()
...
std::future<AddTwoInts_Reply> AddTwoInts_Service::request(
        AddTwoInts_Request& request)
{
    if (!server_)
    {
        ...
        delete promise_;
        promise_ = new std::promise<AddTwoInts_Reply>();
        ...
        fastdds::dds::DataReader* reader)
{
            ...
            reader->take_next_sample(&request, &info))
    {
...
#endif //  if FASTRTPS_VERSION_MINOR < 2
        {
            ...
            std::cout << service_->service_name_
                      << " server: processing incoming request, add "
                      << request.a() << " + " << request.b()
                      << std::endl;

            int64_t sum = request.a() + request.b();
            response.sum(sum);
