// Copyright 2016 Proyectos y Sistemas de Mantenimiento SL (eProsima).
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

/*!
 * @file ArmMotors.h
 * This header file contains the declaration of the described types in the IDL file.
 *
 * This file was generated by the tool gen.
 */

#ifndef _FAST_DDS_GENERATED_ARMMOTORS_H_
#define _FAST_DDS_GENERATED_ARMMOTORS_H_


#include <fastrtps/utils/fixed_size_string.hpp>

#include <stdint.h>
#include <array>
#include <string>
#include <vector>
#include <map>
#include <bitset>

#if defined(_WIN32)
#if defined(EPROSIMA_USER_DLL_EXPORT)
#define eProsima_user_DllExport __declspec( dllexport )
#else
#define eProsima_user_DllExport
#endif  // EPROSIMA_USER_DLL_EXPORT
#else
#define eProsima_user_DllExport
#endif  // _WIN32

#if defined(_WIN32)
#if defined(EPROSIMA_USER_DLL_EXPORT)
#if defined(ARMMOTORS_SOURCE)
#define ARMMOTORS_DllAPI __declspec( dllexport )
#else
#define ARMMOTORS_DllAPI __declspec( dllimport )
#endif // ARMMOTORS_SOURCE
#else
#define ARMMOTORS_DllAPI
#endif  // EPROSIMA_USER_DLL_EXPORT
#else
#define ARMMOTORS_DllAPI
#endif // _WIN32

namespace eprosima {
namespace fastcdr {
class Cdr;
} // namespace fastcdr
} // namespace eprosima


/*!
 * @brief This class represents the structure ArmMotors_Request defined by the user in the IDL file.
 * @ingroup ArmMotors
 */
class ArmMotors_Request
{
public:

    /*!
     * @brief Default constructor.
     */
    eProsima_user_DllExport ArmMotors_Request();

    /*!
     * @brief Default destructor.
     */
    eProsima_user_DllExport ~ArmMotors_Request();

    /*!
     * @brief Copy constructor.
     * @param x Reference to the object ArmMotors_Request that will be copied.
     */
    eProsima_user_DllExport ArmMotors_Request(
            const ArmMotors_Request& x);

    /*!
     * @brief Move constructor.
     * @param x Reference to the object ArmMotors_Request that will be copied.
     */
    eProsima_user_DllExport ArmMotors_Request(
            ArmMotors_Request&& x) noexcept;

    /*!
     * @brief Copy assignment.
     * @param x Reference to the object ArmMotors_Request that will be copied.
     */
    eProsima_user_DllExport ArmMotors_Request& operator =(
            const ArmMotors_Request& x);

    /*!
     * @brief Move assignment.
     * @param x Reference to the object ArmMotors_Request that will be copied.
     */
    eProsima_user_DllExport ArmMotors_Request& operator =(
            ArmMotors_Request&& x) noexcept;

    /*!
     * @brief Comparison operator.
     * @param x ArmMotors_Request object to compare.
     */
    eProsima_user_DllExport bool operator ==(
            const ArmMotors_Request& x) const;

    /*!
     * @brief Comparison operator.
     * @param x ArmMotors_Request object to compare.
     */
    eProsima_user_DllExport bool operator !=(
            const ArmMotors_Request& x) const;

    /*!
     * @brief This function sets a value in member arm
     * @param _arm New value for member arm
     */
    eProsima_user_DllExport void arm(
            bool _arm);

    /*!
     * @brief This function returns the value of member arm
     * @return Value of member arm
     */
    eProsima_user_DllExport bool arm() const;

    /*!
     * @brief This function returns a reference to member arm
     * @return Reference to member arm
     */
    eProsima_user_DllExport bool& arm();


    /*!
    * @brief This function returns the maximum serialized size of an object
    * depending on the buffer alignment.
    * @param current_alignment Buffer alignment.
    * @return Maximum serialized size.
    */
    eProsima_user_DllExport static size_t getMaxCdrSerializedSize(
            size_t current_alignment = 0);

    /*!
     * @brief This function returns the serialized size of a data depending on the buffer alignment.
     * @param data Data which is calculated its serialized size.
     * @param current_alignment Buffer alignment.
     * @return Serialized size.
     */
    eProsima_user_DllExport static size_t getCdrSerializedSize(
            const ArmMotors_Request& data,
            size_t current_alignment = 0);


    /*!
     * @brief This function serializes an object using CDR serialization.
     * @param cdr CDR serialization object.
     */
    eProsima_user_DllExport void serialize(
            eprosima::fastcdr::Cdr& cdr) const;

    /*!
     * @brief This function deserializes an object using CDR serialization.
     * @param cdr CDR serialization object.
     */
    eProsima_user_DllExport void deserialize(
            eprosima::fastcdr::Cdr& cdr);



    /*!
     * @brief This function returns the maximum serialized size of the Key of an object
     * depending on the buffer alignment.
     * @param current_alignment Buffer alignment.
     * @return Maximum serialized size.
     */
    eProsima_user_DllExport static size_t getKeyMaxCdrSerializedSize(
            size_t current_alignment = 0);

    /*!
     * @brief This function tells you if the Key has been defined for this type
     */
    eProsima_user_DllExport static bool isKeyDefined();

    /*!
     * @brief This function serializes the key members of an object using CDR serialization.
     * @param cdr CDR serialization object.
     */
    eProsima_user_DllExport void serializeKey(
            eprosima::fastcdr::Cdr& cdr) const;

private:

    bool m_arm;

};
/*!
 * @brief This class represents the structure ArmMotors_Response defined by the user in the IDL file.
 * @ingroup ArmMotors
 */
class ArmMotors_Response
{
public:

    /*!
     * @brief Default constructor.
     */
    eProsima_user_DllExport ArmMotors_Response();

    /*!
     * @brief Default destructor.
     */
    eProsima_user_DllExport ~ArmMotors_Response();

    /*!
     * @brief Copy constructor.
     * @param x Reference to the object ArmMotors_Response that will be copied.
     */
    eProsima_user_DllExport ArmMotors_Response(
            const ArmMotors_Response& x);

    /*!
     * @brief Move constructor.
     * @param x Reference to the object ArmMotors_Response that will be copied.
     */
    eProsima_user_DllExport ArmMotors_Response(
            ArmMotors_Response&& x) noexcept;

    /*!
     * @brief Copy assignment.
     * @param x Reference to the object ArmMotors_Response that will be copied.
     */
    eProsima_user_DllExport ArmMotors_Response& operator =(
            const ArmMotors_Response& x);

    /*!
     * @brief Move assignment.
     * @param x Reference to the object ArmMotors_Response that will be copied.
     */
    eProsima_user_DllExport ArmMotors_Response& operator =(
            ArmMotors_Response&& x) noexcept;

    /*!
     * @brief Comparison operator.
     * @param x ArmMotors_Response object to compare.
     */
    eProsima_user_DllExport bool operator ==(
            const ArmMotors_Response& x) const;

    /*!
     * @brief Comparison operator.
     * @param x ArmMotors_Response object to compare.
     */
    eProsima_user_DllExport bool operator !=(
            const ArmMotors_Response& x) const;

    /*!
     * @brief This function sets a value in member result
     * @param _result New value for member result
     */
    eProsima_user_DllExport void result(
            bool _result);

    /*!
     * @brief This function returns the value of member result
     * @return Value of member result
     */
    eProsima_user_DllExport bool result() const;

    /*!
     * @brief This function returns a reference to member result
     * @return Reference to member result
     */
    eProsima_user_DllExport bool& result();


    /*!
    * @brief This function returns the maximum serialized size of an object
    * depending on the buffer alignment.
    * @param current_alignment Buffer alignment.
    * @return Maximum serialized size.
    */
    eProsima_user_DllExport static size_t getMaxCdrSerializedSize(
            size_t current_alignment = 0);

    /*!
     * @brief This function returns the serialized size of a data depending on the buffer alignment.
     * @param data Data which is calculated its serialized size.
     * @param current_alignment Buffer alignment.
     * @return Serialized size.
     */
    eProsima_user_DllExport static size_t getCdrSerializedSize(
            const ArmMotors_Response& data,
            size_t current_alignment = 0);


    /*!
     * @brief This function serializes an object using CDR serialization.
     * @param cdr CDR serialization object.
     */
    eProsima_user_DllExport void serialize(
            eprosima::fastcdr::Cdr& cdr) const;

    /*!
     * @brief This function deserializes an object using CDR serialization.
     * @param cdr CDR serialization object.
     */
    eProsima_user_DllExport void deserialize(
            eprosima::fastcdr::Cdr& cdr);



    /*!
     * @brief This function returns the maximum serialized size of the Key of an object
     * depending on the buffer alignment.
     * @param current_alignment Buffer alignment.
     * @return Maximum serialized size.
     */
    eProsima_user_DllExport static size_t getKeyMaxCdrSerializedSize(
            size_t current_alignment = 0);

    /*!
     * @brief This function tells you if the Key has been defined for this type
     */
    eProsima_user_DllExport static bool isKeyDefined();

    /*!
     * @brief This function serializes the key members of an object using CDR serialization.
     * @param cdr CDR serialization object.
     */
    eProsima_user_DllExport void serializeKey(
            eprosima::fastcdr::Cdr& cdr) const;

private:

    bool m_result;

};

#endif // _FAST_DDS_GENERATED_ARMMOTORS_H_

